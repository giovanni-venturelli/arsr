...
<xsl:template match="/">
	...
	<body>
		...
		<div id="menu">
		<ul>
			<xsl:for-each select="document('../data/menu.xml')/menu/page">
				<xsl:choose>
				<xsl:when test="not(@id='attrezzature')">
					<li class="menulist">
						<a>
		...
		<div id="content">
			<xsl:for-each select="Database/Attrezzatura">
		...
		<div id="footer">
		©2014 Y-Tech Giovanni Venturelli, Francesco Agostini, Matteo Gnoato, Nicolò Scapin</div>
		</div>
	</body>
	</html>
</xsl:template>
